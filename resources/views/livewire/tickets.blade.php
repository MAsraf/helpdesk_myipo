<div class="w-full flex flex-col justify-start items-start gap-5">
    <div class="w-full flex md:flex-row flex-col justify-between items-start gap-2">
        ...
        @can('Create tickets')
            ...
            <button id="myBtn" type="button" wire:click="createTicket" class="bg-primary-700 text-white hover:bg-primary-800 px-4 py-2 rounded-lg shadow hover:shadow-lg
            text-base">
            @lang('Create a new ticket')</button>
        @endCan
    </div>
            ...
            <div class="w-full flex flex-col gap-5" wire:ignore>
                <div id="filter-accordion" data-accordion="collapse">
                    ...
                    <div id="filter-accordion-body" class="hidden" aria-labelledby="filter-accordion-heading">
                        <div id="ticketModal" class="hidden w-full flex flex-row justify-start items-center p-5 border border-gray-200">
                            <form wire:submit.prevent="search"
                                ...
                                <div class="flex flex-row items-center gap-1">
                                    ...
                                    <button type="button" title="reset" wire:click="resetFilters"
    ...
    <div wire:ignore.self id="myModal" tabindex="-1" aria-hidden="true"
         class="modal overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 flex items-center
         justify-center w-full md:inset-0 md:h-full">
            <!-- Modal content -->
            <div class="modal-content relative p-1 w-full max-w-4xl  h-full md:h-auto bg-white rounded-lg shadow dark:bg-gray-700">
                <!-- Modal header -->
                <div class="modal-header flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
                    <h3 class="modal-body text-xl font-semibold text-gray-900 dark:text-white">
                        @lang('Create a new ticket')
                    </h3>
                    <button id="myBtnClosed" wire:click="cancelTicket" type="button" 
    ...
    <button type="button" title="hidden" class="hidden" data-toggle="modal" id="toggleTicketModal" data-target="#ticketModal"></button>
...
var btnClose = document.getElementById("myBtnClosed");
    ...
    modal.style.display = "none";
            ...
            window.addEventListener('ticketUrlCopied', (event) => {
                if (event.detail && event.detail.url) {
                    ...
                    console.error('Event detail URL not found');
                }
            });
